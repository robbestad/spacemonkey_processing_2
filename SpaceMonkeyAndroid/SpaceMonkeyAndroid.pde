...
void setup()
{
  ...
  bgdead = loadImage("bgdead480x720.png");
  bgstart = loadImage("bgstart480x720.png");
...
void draw()
{
  ...
  // width distribution
  int[] widthdist = { 125,155,175,205,245 };
  shuffle(widthdist);
  ...
  for (int i = 0; i < bananaList.size(); i++) {
    ...
    if (checkHit(banana, monkey)) {
      if (health > 0) {
        ...
        if(debug) println("remove banana");
  ...
  for (int i = 0; i < asteroidList.size(); i++) {
    ...
    if (checkHit(asteroid, monkey)) {
      if (health > 0) {
        ...
        //asteroidList[i] = new PImage[0];
        if(debug) println("remove asteroid");
   ...
   for (int i = 0; i < asteroidList.size(); i++) {
    ...
    for (int j = 0; j < projectileList.size(); j++) {
      ...
      if (checkHit(proj, asteroid)) {
        ...
        if(debug) println("remove projectile");
        projectileList.remove(j--);
  ...
  for (int i = 0; i < enemyProjectileList.size(); i++) {
    ...
    proj.disp();
    if (proj.y + proj.h < 0){
      //enemyProjectileList[i] = new PImage[0];
  ...
  for (int i = 0; i < explosionList.size(); i++) {
    ...
    explosion.disp();
    if(explosion.killed==1){
      //explosionList[i] = new PImage[0];
